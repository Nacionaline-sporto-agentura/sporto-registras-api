...
} = require('./20240916135056_publicViewsDistinct');
  ...
  FROM
    (
      ...
      FROM
        ...
        LEFT JOIN LATERAL jsonb_array_elements(sbs.sport_base_space_sport_types) AS sport_type(id) ON TRUE
        LEFT JOIN types_sport_types tst ON (sport_type.id) :: text :: int = tst.id
    ) sbsst
  GROUP BY
    sbsst.id
),
mapped_additional_values AS (
  ...
  FROM
    sports_bases_spaces sbs
    LEFT JOIN LATERAL jsonb_each_text(sbs.additional_values) AS additional_value(id, value) ON TRUE
    LEFT JOIN fields_translates ft ON (additional_value.id) :: text :: int = ft.id
  GROUP BY
    sbs.id
),
mapped_base_tenants AS (
    ...
    coalesce(
      jsonb_agg(
        jsonb_build_object(
          ...
          'basis',
          CASE
            WHEN tsbtb.id IS NOT NULL THEN jsonb_build_object('id', tsbtb.id, 'name', tsbtb.name)
          END,
          'code',
          sbt.company_code
        )
  ...
  WHERE
    sbt.deleted_at IS NULL
  GROUP BY
    sbt.sport_base_id
),
mapped_bases_public_photos AS (
  SELECT
    sb.id,
    jsonb_agg(DISTINCT photo.item) filter (
      WHERE
        photo.item ->> 'public' = 'true'
    ) AS photos
  FROM
    sports_bases sb
...
mapped_space_public_photos AS (
  ...
  FROM
    sports_bases_spaces sbs
    LEFT JOIN LATERAL jsonb_array_elements(sbs.photos) AS photo(item) ON TRUE
  GROUP BY
    sbs.id
),
mapped_sports_spaces AS (
  SELECT
    sbs.sport_base_id,
    coalesce(
      jsonb_agg(DISTINCT st.item) FILTER (
        WHERE
          st.item IS NOT NULL
      ),
      '[]' :: jsonb
  ...
  FROM
    ...
    LEFT JOIN mapped_space_public_photos mspp ON mspp.id = sbs.id
    LEFT JOIN LATERAL jsonb_array_elements(mst.sport_types) AS st(item) ON TRUE
  WHERE
...
SELECT
  sb.id,
  sb.name,
  coalesce(mbpp.photos, '[]' :: jsonb) AS photos,
  sb.web_page,
  jsonb_build_object(
    'municipality',
  ...
  sb.saunas,
  coalesce(sb.public_wifi, FALSE),
  sb.email,
  sb.phone,
  sb.geom,
  mt.tenant,
  coalesce(mss.spaces, '[]' :: jsonb) AS spaces,
  coalesce(mss.sport_types, '[]' :: jsonb) AS sport_types,
  mss.construction_date,
  coalesce(mbt.tenants, '[]' :: jsonb) AS tenants,
  CASE
    WHEN tsbt.id IS NOT NULL THEN jsonb_build_object('id', tsbt.id, 'name', tsbt.name)
  END AS "type"
FROM
  sports_bases sb
  ...
  LEFT JOIN mapped_bases_public_photos mbpp ON mbpp.id = sb.id
WHERE
